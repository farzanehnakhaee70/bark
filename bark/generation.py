...
class BarkCoarse(torch.nn.Module):
    def __init__(
        self, 
        use_gpu, 
        use_small
        ...
        _load_model_f = funcy.partial(_load_model, model_type="coarse", use_small=use_small)
        device = _grab_best_device(use_gpu=use_gpu)
        if OFFLOAD_CPU:
            self.model_device = device
            device = "cpu"
        ckpt_path = _get_ckpt_path("coarse", use_small=use_small)
        self.model = _load_model_f(ckpt_path, device)
    ...
    def generate(
        ...
        # load models if not yet exist
        if OFFLOAD_CPU:
            self.model.to(self.models_devices)
